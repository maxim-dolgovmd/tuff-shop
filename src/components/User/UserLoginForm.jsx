...
import { useLoginUserMutation, useProfileUserMutation } from "../../api/usersApi";

const UserLoginForm = () => {

  // const [CreateUser, {data: newUser}] = useCreateUsersMutation()
  const [LoginUser] = useLoginUserMutation()
  const [AccessToken] = useProfileUserMutation()
  ...
  const OnSubmit = handleSubmit(async (data) => {
    ...
    const {data: userAuth} = await AccessToken(login?.data?.access_token)
  ...
  return (
      <div className={styled.wrapper} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styled.title}>Log in</div>
        ...
        <form className={styled.form}>
          ...
          <div className={styled.link} onClick={() => dispatch(setFormType('signup'))}> Create an account</div>
